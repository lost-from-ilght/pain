...
(function () {
  ...
  waitForAdminShell().then(() => {
    ...
    const USERS_TABLE_CONFIG = {
      ...
      actions: [
        { label: "Edit", className: "btn btn-sm btn-primary me-1", onClick: "handleUserEdit" },
        { label: "Delete", className: "btn btn-sm btn-primary me-1", onClick: "handleUserDelete", confirm: "Delete this user?" },
        { label: "View", className: "btn btn-sm btn-outline-primary", onClick: "handleUserView" }
      ]
    };

    // Action handlers
    window.handleUserEdit = (row) => {
      console.log("Edit user:", row);
    };

    window.handleUserDelete = (row) => {
      console.log("Delete user:", row);
    };

    window.handleUserView = (row) => {
    ...
    /**
     * Render users table with filters and pagination
    ...
    async function render(paginationOffset = 0) {
      ...
      const [apiResponse, totalCount] = await Promise.all([
        window.DataService.get("users", {
          filters: activeFilters,
          pagination: { limit: 999, offset: paginationOffset }
        }),
        window.ApiService.getTotalCount("users", activeFilters)
      ]);
      // Store current data for sorting
      currentData = apiResponse.items || [];
      // Generate table HTML using Table component
      const tableHtml = window.Table.create(USERS_TABLE_CONFIG, currentData);
    ...
    document.body.addEventListener("table:sort", (e) => {
      ...
      if (tableId !== USERS_TABLE_CONFIG.id) return;
      const sortedData = window.Table.sortData(currentData, column, direction);
      const tbody = document.querySelector(`#${tableId} tbody`);
      if (tbody) {
        const rowsHtml = window.Table.createRows(USERS_TABLE_CONFIG, sortedData);
