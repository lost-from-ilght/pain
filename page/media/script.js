...
(function () {
  ...
  waitForAdminShell().then(() => {
    ...
    const MEDIA_TABLE_CONFIG = {
      ...
      actions: [
        { label: "Edit", className: "btn btn-sm btn-primary me-1", onClick: "handleMediaEdit" },
        { label: "Delete", className: "btn btn-sm btn-primary me-1", onClick: "handleMediaDelete", confirm: "Delete this media?" },
        { label: "View", className: "btn btn-sm btn-outline-primary", onClick: "handleMediaView" }
      ]
    };

    // Action handlers
    window.handleMediaEdit = (row) => {
      console.log("Edit media:", row);
    };

    window.handleMediaDelete = (row) => {
      console.log("Delete media:", row);
    };

    window.handleMediaView = (row) => {
    ...
    /**
     * Render media table with filters and pagination
    ...
    async function render(paginationOffset = 0) {
      ...
      const [apiResponse, totalCount] = await Promise.all([
        window.DataService.get("media", {
          filters: activeFilters,
          pagination: { limit: 999, offset: paginationOffset }
        }),
        window.ApiService.getTotalCount("media", activeFilters)
      ]);
      // Store current data for sorting
      currentData = apiResponse.items || [];
      // Generate table HTML using Table component
      const tableHtml = window.Table.create(MEDIA_TABLE_CONFIG, currentData);
    ...
    document.body.addEventListener("table:sort", (e) => {
      ...
      if (tableId !== MEDIA_TABLE_CONFIG.id) return;
      const sortedData = window.Table.sortData(currentData, column, direction);
      const tbody = document.querySelector(`#${tableId} tbody`);
      if (tbody) {
        const rowsHtml = window.Table.createRows(MEDIA_TABLE_CONFIG, sortedData);
