...
(function () {
  ...
  waitForAdminShell().then(() => {
    ...
    const PRODUCTS_TABLE_CONFIG = {
      ...
      actions: [
        { label: "Edit", className: "btn btn-sm btn-primary me-1", onClick: "handleProductEdit" },
        { label: "Delete", className: "btn btn-sm btn-primary me-1", onClick: "handleProductDelete", confirm: "Delete this product?" },
        { label: "View", className: "btn btn-sm btn-outline-primary", onClick: "handleProductView" }
      ]
    };

    // Action handlers
    window.handleProductEdit = (row) => {
      console.log("Edit product:", row);
      // TODO: Implement edit functionality
    };

    window.handleProductDelete = (row) => {
    ...
    function attachChipRemovalHandlers(containerElement) {
      ...
      containerElement.querySelectorAll(".filter-chip .btn-close").forEach((closeButton) => {
        ...
        closeButton.addEventListener("click", () => {
          // Find closest filter chip element
          const filterChipElement = closeButton.closest(".filter-chip");
          // Get filter key from chip data attribute
          const filterKey = filterChipElement?.getAttribute("data-chip");
    ...
    /**
     * Render products table with filters and pagination
    ...
    async function render(paginationOffset = 0) {
      ...
      const tableHtml = window.Table.create(PRODUCTS_TABLE_CONFIG, currentData);
    ...
    document.body.addEventListener("table:sort", (e) => {
      ...
      if (tableId !== PRODUCTS_TABLE_CONFIG.id) return;
      ...
      if (tbody) {
        const rowsHtml = window.Table.createRows(PRODUCTS_TABLE_CONFIG, sortedData);
