/**
 * Edge Tests - Cart Class
 * 
 * This is a comprehensive edge test page for cart functionality.
 * 
 * STRUCTURE:
 * ==========
    ...
    function getBaseUrl() {
      ...
      try {
        ...
        if (configScriptElement) {
          ...
          const cartConfig = pageConfig["cart"];
          if (cartConfig && cartConfig[currentEnvironment] && cartConfig[currentEnvironment].endpoint) {
            const endpointUrl = cartConfig[currentEnvironment].endpoint;
        ...
        console.warn("[Edge Tests Cart] Could not parse API config, using default base URL:", configError);
      }
      console.log("[Edge Tests Cart] Using base URL:", baseUrl);
      return baseUrl;
    }
      ...
      return `
        <div class="demo-section index-section">
          ...
          <a href="#test-scenario-1" class="index-link">
            <i class="bi bi-play-circle"></i> Test Scenario 1: Create Cart Item
          </a>
          <a href="#test-scenario-2" class="index-link">
            <i class="bi bi-play-circle"></i> Test Scenario 2: Get Cart Items by User ID (with Input)
          </a>
          <a href="#test-scenario-3" class="index-link">
            <i class="bi bi-play-circle"></i> Test Scenario 3: Get Cart Items by Status (with Select)
          </a>
          <a href="#test-scenario-4" class="index-link">
            <i class="bi bi-play-circle"></i> Test Scenario 4: Update Cart Item (with Inputs)
          </a>
...
apiHandler.handleRequest(apiParams);
    ...
    async function testScenario(scenarioId, method, endpoint, payload) {
      ...
      try {
        ...
        if (inputValues.cartId) {
          finalEndpoint = finalEndpoint.replace('{cartId}', inputValues.cartId);
          finalEndpoint = finalEndpoint.replace('{id}', inputValues.cartId);
        }
        if (inputValues.id) {
        ...
        console.error(`[Edge Tests Cart] Error in scenario ${scenarioId}:`, error);
      }
    }
    ...
    async function cleanupTestData() {
      ...
      try {
        const baseUrl = getBaseUrl();
        const cleanupUrl = `${baseUrl}/cart/cleanup`;
        const apiHandler = new APIHandler();
        const cleanupParams = {
        ...
        `;
        console.error('[Edge Tests Cart] Cleanup error:', error);
      }
    }
      ...
      const pageHtml = `
        ...
        <div class="demo-section">
          ...
          ${createTestScenarioSection(
            "1",
            "Test Scenario 1: Create Cart Item",
            "This test demonstrates how to create a new cart item using a POST request. The request includes testing: true to indicate this is a test request.",
            "POST",
            "/cart/create",
            {
              userId: "user-123",
            ...
            [
              "Go to database and verify new cart record was created",
              "Check that 'userId' field matches 'user-123'",
              "Check that 'quantity' field is 2",
          ...
          ${createTestScenarioSection(
            "2",
            "Test Scenario 2: Get Cart Items by User ID",
            "This test demonstrates how to retrieve cart items for a specific user using a GET request with user ID input field.",
            "GET",
            "/cart/user/{userId}/list",
            null,
            [
              "Enter a user ID in the input field above",
              "Click 'Test API Call' to execute the request",
              "Verify API response contains cart items for the specified user",
          ...
          ${createTestScenarioSection(
            "3",
            "Test Scenario 3: Get Cart Items by Status",
            "This test demonstrates how to retrieve cart items filtered by status using a GET request with select dropdown.",
            "GET",
            "/cart/list",
            null,
            [
              ...
              "Click 'Test API Call' to execute the request",
              "Verify API response contains only cart items with selected status",
          ...
          ${createTestScenarioSection(
            "4",
            "Test Scenario 4: Update Cart Item with Inputs",
            "This test demonstrates how to update an existing cart item using a PUT request with user ID input and status select dropdown.",
            "PUT",
            "/cart/update/{userId}",
            {
              quantity: 3
            ...
            [
              ...
              "Click 'Test API Call' to execute the request",
              "Go to database and find the updated cart item",
        ...
        if (event.target.classList.contains("test-scenario-btn")) {
          ...
          if (payloadString && payloadString !== "null") {
            try {
              ...
              console.error("[Edge Tests Cart] Could not parse payload:", parseError);
            }
          }
